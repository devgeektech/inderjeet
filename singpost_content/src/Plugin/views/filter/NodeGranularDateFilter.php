...
namespace Drupal\singpost_content\Plugin\views\filter;
...
class NodeGranularDateFilter extends FilterPluginBase{
	...
	private function getYearsOptions($emptyOption = FALSE){
		...
		$anys = range($this->options['granular_year_from'], $this->options['granular_year_until']);
	...
	public function query(){
		...
		if (!$this->options['exposed']){
			...
			$this->queryFilter($this->options['granular_field_name'],
				$this->options['node_year'] . '-' . $this->options['node_month']);
		}else{
			// Exposed value.
		...
		if ($year != 'All' && $month != 'All'){
			...
			if ($fieldName == 'created' || $fieldName == 'changed'){
				$this->query->addWhereExpression(0,
					"from_unixtime(node_field_data.{$fieldName}, '%m') = {$monthWithZero}", []);
			}else{
				$this->query->addTable("node__{$fieldName}");
				$this->query->addWhereExpression(0,
					"date_format(node__{$fieldName}.{$fieldName}_value, '%m') = {$monthWithZero}",
					[]);
			}
		}
